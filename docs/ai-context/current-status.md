...
**Diagnóstico definitivo (2026-05-16 — `1.0.53+53`):** A causa raiz **não era timing** — era o `SubscriptionService.instance` como `refreshListenable` do **GoRouter**. O SDK do RevenueCat dispara `addCustomerInfoUpdateListener` automaticamente em toda retomada do app (`applicationDidBecomeActive`). Isso chamava `notifyListeners()` no `SubscriptionService` → o GoRouter chamava `go(currentUri)` internamente (renavegação) → enquanto o iOS ainda estava no frame de retomada → **crash nativo**. Rotas empilhadas não crashavam porque o `go` da rota-base (NavBarPage) não afetava a rota visível (Contato etc.). Tela de vídeo não crashava porque usa `context.goNamed` (substitui a pilha) e NavBarPage não estava montada. **Fix aplicado em `1.0.53+53`:** `SubscriptionService` agora expoe `premiumStatusNotifier` (`ValueNotifier<bool>`) que **só notifica quando `hasPremiumAccess` muda de valor** — nunca em refresh de rotina. `nav.dart` usa `premiumStatusNotifier` no `refreshListenable` em vez de `SubscriptionService.instance`. Compra, restauração e refresh explícito ainda chamam `_syncPremiumNotifier()` para disparar GoRouter quando relevante. **Validar** em novo build TestFlight.
...
**EN:** TestFlight validated. **Working:** annual IAP, coupon, manage-subscription states, gates, schedule. **Monthly row on paywall:** **not an app bug** — operator ships **annual-only** subscription in App Store Connect for first submission. **P0 (updated 2026-05-16):** crash **only on NavBar tabs** (Home, Favorites, History, Settings) when returning from background — NOT on pushed routes. Contact→Mail→return: fixed in `1.0.51+51`. **Root cause:** `addPostFrameCallback` fires before `UIWindow.isKeyWindow` is true; `Purchases.showInAppMessages()` (StoreKit) called too early → native crash. **Fix:** replaced with `Future.delayed(800ms)` to let iOS fully resume before native calls. **Re-test** on TestFlight. **$69.99 in manage screen:** not a bug — RevenueCat shows price from device's store account/region.
...
**PT-BR:** **`pubspec.yaml`** **`1.0.51+51`**; **`app_build_metadata`** **1.0.51** / **14/05/2026**. **Cupom (Android + iOS):** `AccessCodeService.redeem` com **single-flight** (`_ongoingRedeem`) para não disparar dois POSTs; paywall com **`submitLocked`** antes do `await` — evita mensagem falsa *"Código inválido ou já utilizado"* com sucesso. **iOS P0 (mitigação parcial):** em `NavBarPage.didChangeAppLifecycleState(resumed)`, RevenueCat `showInAppMessages`, `_checkParentalLimits` e `_openProfileSelectionIfNeeded` só após **post-frame** e se **`ModalRoute.isCurrent`** — resolve crash em subrotas empilhadas (ex.: Contato → Mail → voltar). **Crash em NavBar tabs** (Home, Favorítos, Histórico, Configurações) permanecia: ver fix de `2026-05-16` abaixo.

**EN:** **`1.0.51+51`**, metadata **1.0.51** / **14/05/2026**. **Coupon:** single-flight redeem + dialog submit lock. **iOS:** defers resume side-effects until `ModalRoute.isCurrent` — fixes stacked-route crash (e.g. Contact/Mail). **NavBar tabs crash persisted:** see 2026-05-16 fix.

## Fix definitivo — iOS crash NavBar tabs causa raiz (2026-05-16, `1.0.53+53`)

**PT-BR:** Identificada a causa raiz do crash iOS ao retomar de segundo plano nas tabs do NavBar. O `SubscriptionService.instance` estava no `refreshListenable` do GoRouter. O SDK do RevenueCat atualiza `CustomerInfo` automaticamente em todo `applicationDidBecomeActive` → `addCustomerInfoUpdateListener` → `notifyListeners()` → GoRouter chama `go(currentUri)` durante o frame de retomada do iOS → crash nativo. Fix: `SubscriptionService` ganhou `premiumStatusNotifier` (`ValueNotifier<bool>`) que só muda quando `hasPremiumAccess` muda. `nav.dart` usa `premiumStatusNotifier` no `refreshListenable`. Compra/restauração/coupon ainda disparam GoRouter quando relevante via `_syncPremiumNotifier()`. Versão: `1.0.53+53`.

**EN:** Root cause found: `SubscriptionService.instance` was the GoRouter `refreshListenable`. RevenueCat SDK fires `addCustomerInfoUpdateListener` on every `applicationDidBecomeActive` (every resume) → `notifyListeners()` → GoRouter calls `go(currentUri)` during iOS resume frame → native crash on NavBar tabs. Fix: `SubscriptionService` now exposes `premiumStatusNotifier` (`ValueNotifier<bool>`) that only fires when `hasPremiumAccess` actually changes. `nav.dart` uses `premiumStatusNotifier` in `refreshListenable`. Purchase/restore/coupon still trigger GoRouter via `_syncPremiumNotifier()`. Version: `1.0.53+53`.

...
Last updated: 2026-05-16 — fix P0 iOS crash (NavBar tabs) + textos ASO App Store atualizados
...
**Diagnóstico (2026-05-16):** o `addPostFrameCallback` da `1.0.51+51` garantia o guard de rota (`ModalRoute.isCurrent`), mas o **primeiro frame** pode ser desenhado antes de `UIWindow.isKeyWindow == true` no iOS. Quando NavBarPage é a rota corrente, o guard passava, e `Purchases.showInAppMessages()` (StoreKit) era chamado com a janela ainda em estado "inactive" → crash nativo. **Fix aplicado:** substituído `addPostFrameCallback` por `Future.delayed(800ms)` — dá ao iOS tempo para completar a transição de retomada antes de qualquer chamada nativa (RevenueCat/StoreKit, diálogos, `pushNamed`). **Validar** em novo build TestFlight.
...
## Fix pendente de commit — iOS crash NavBar tabs (2026-05-16)

**PT-BR:** Substituído `WidgetsBinding.instance.addPostFrameCallback` por `Future.delayed(800ms)` em `NavBarPage.didChangeAppLifecycleState(resumed)` em **`lib/main.dart`**. O `postFrameCallback` disparava antes do `UIWindow.isKeyWindow` ficar `true` no iOS; na retomada via NavBar tab (sem rota empilhada), o guard `ModalRoute.isCurrent` era verdadeiro e `Purchases.showInAppMessages()` chegava antes da janela estar ativa → crash nativo. O delay de 800ms garante que a retomada do iOS completou antes de qualquer chamada nativa. Ainda não commitado — aguarda autorização do operador.

**EN:** Replaced `addPostFrameCallback` with `Future.delayed(800ms)` in `NavBarPage.didChangeAppLifecycleState(resumed)` (`lib/main.dart`). Previous guard only blocked stacked routes; on NavBar tabs (no route on top) `ModalRoute.isCurrent` was true, so `Purchases.showInAppMessages()` (StoreKit) fired before `UIWindow.isKeyWindow` was true → native crash. The 800ms delay ensures iOS completes its resume transition first. **Not yet committed — awaiting operator authorization.**
